...
.co-rec-title { font-size:28px; font-weight:800; color:var(--inv-navy); letter-spacing:-.5px; margin:0; line-height:1; }
.co-rec-subtitle { font-size:14px; color:var(--inv-s4); margin:6px 0 0; line-height:1.5; }
.co-rec-btn-primary { display:inline-flex; align-items:center; gap:7px; background:var(--inv-indigo); color:#fff; border:none; padding:10px 18px; border-radius:8px; font-size:14px; font-weight:600; cursor:pointer; transition:opacity .15s; }
.co-rec-btn-primary:hover { opacity:.88; }
.co-rec-btn-ghost { background:none; border:1.5px solid var(--inv-s2); color:var(--inv-s6); padding:10px 18px; border-radius:8px; font-size:13px; font-weight:500; cursor:pointer; transition:border-color .15s; }
.co-rec-btn-ghost:hover { border-color:var(--inv-indigo); color:var(--inv-indigo); }

.co-rec-tabs { display:flex; gap:6px; flex-wrap:wrap; margin-bottom:20px; }
.co-rec-tab { display:flex; align-items:center; gap:7px; padding:7px 16px; border-radius:20px; font-size:13px; font-weight:500; color:var(--inv-s4); border:1.5px solid var(--inv-s2); background:#fff; cursor:pointer; transition:all .12s; }
.co-rec-tab:hover { border-color:var(--inv-indigo); color:var(--inv-indigo); }
.co-rec-tab.active { background:var(--inv-indigo); border-color:var(--inv-indigo); color:#fff; font-weight:600; }
.co-rec-tab-count { font-size:11px; font-weight:700; background:var(--inv-s1); color:var(--inv-s4); border-radius:999px; padding:1px 7px; min-width:20px; text-align:center; }
...
.co-rec-page-btn:hover:not(:disabled) { border-color:var(--inv-indigo); color:var(--inv-indigo); background:#EEF2FF; }
.co-rec-page-btn.active { background:var(--inv-indigo); color:#fff; border-color:var(--inv-indigo); }
...
.co-rec-table-wrap { background:#fff; border-radius:12px; border:1px solid var(--inv-s2); overflow-x:auto; overflow-y:hidden; box-shadow: 0 1px 3px rgba(26,26,46,.04), 0 6px 24px rgba(26,26,46,.06); }
.co-rec-table { width:100%; min-width:680px; border-collapse:collapse; }
.co-rec-table th { text-align:left; padding:11px 16px; font-size:11px; font-weight:700; color:var(--inv-s4); text-transform:uppercase; letter-spacing:.5px; border-bottom:1px solid var(--inv-s2); background:var(--inv-s1); }
.co-rec-table td { padding:14px 16px; border-bottom:1px solid var(--inv-s2); font-size:14px; color:var(--inv-s6); }
.co-rec-table tr:last-child td { border-bottom:none; }
.co-rec-table tr:hover td { background:var(--inv-s1); }
.co-rec-client { font-weight:500; color:var(--inv-navy); }
.co-rec-amount { font-weight:700; color:var(--inv-navy); }
.co-rec-next { color:var(--inv-s5); font-size:13px; }
...
.co-rec-act-edit { background:var(--inv-s1); color:var(--inv-s5); }
.co-rec-act-pause { background:#FEF3C7; color:#92400E; }
.co-rec-act-resume { background:#D1FAE5; color:#065F46; }
.co-rec-act-cancel { background:#FEF2F2; color:var(--inv-red); }

.co-rec-empty { background:#fff; border:1.5px solid var(--inv-s2); border-radius:12px; padding:56px 32px; text-align:center; }
.co-rec-empty-icon { color:var(--inv-s3); margin:0 auto 16px; display:block; }
.co-rec-empty-title { font-family:'Archivo', -apple-system, BlinkMacSystemFont, sans-serif; font-size:20px; color:var(--inv-navy); margin:0 0 8px; }
.co-rec-empty-sub { font-size:14px; color:var(--inv-s5); max-width:380px; margin:0 auto; line-height:1.6; }

.co-rec-loading { text-align:center; padding:48px; color:var(--inv-s4); }
...
.co-rec-back { display:inline-flex; align-items:center; gap:6px; color:var(--inv-s4); font-size:13px; font-weight:500; cursor:pointer; border:none; background:none; padding:0; margin-bottom:20px; transition:color .15s; }
.co-rec-back:hover { color:var(--inv-indigo); }
.co-rec-editor-title { font-size:24px; font-weight:800; color:var(--inv-navy); margin:0 0 24px; }
.co-rec-card { background:#fff; border:1px solid var(--inv-s2); border-radius:12px; padding:24px; margin-bottom:16px; }
.co-rec-card-title { font-size:13px; font-weight:700; color:var(--inv-s5); text-transform:uppercase; letter-spacing:.5px; margin:0 0 16px; padding-bottom:12px; border-bottom:1px solid var(--inv-s2); }
.co-rec-field { margin-bottom:16px; }
.co-rec-field label { display:block; font-size:13px; font-weight:600; color:var(--inv-s6); margin-bottom:6px; }
.co-rec-field input:not([type="radio"]):not([type="checkbox"]), .co-rec-field select, .co-rec-field textarea { width:100%; padding:9px 12px; border:1.5px solid var(--inv-s2); border-radius:8px; font-size:14px; line-height:20px; color:var(--inv-navy); background:#fff; box-sizing:border-box; transition:border-color .15s; }
...
.co-rec-end-toggle input[type="radio"] { width:16px; height:16px; min-width:16px; min-height:16px; margin:0; flex:none; accent-color:var(--inv-indigo); cursor:pointer; }
...
.co-rec-field input:not([type="radio"]):not([type="checkbox"]):focus, .co-rec-field select:focus, .co-rec-field textarea:focus { outline:none; border-color:var(--inv-indigo); box-shadow:0 0 0 3px rgba(99,102,241,.1); }
...
.co-rec-help { font-size:12px; color:var(--inv-s4); margin:6px 0 0; line-height:1.5; }
...
.co-rec-item input:focus { outline:none; border-color:var(--inv-indigo); box-shadow:0 0 0 3px rgba(99,102,241,.1); }
.co-rec-item-del { background:none; border:none; color:var(--inv-s3); cursor:pointer; font-size:18px; line-height:1; padding:4px; border-radius:6px; transition:color .15s; }
.co-rec-item-del:hover { color:var(--inv-red); }
.co-rec-add-item { background:none; border:1.5px dashed var(--inv-s3); color:var(--inv-s5); border-radius:8px; padding:8px 12px; font-size:13px; font-weight:600; cursor:pointer; width:100%; text-align:center; transition:border-color .15s; }
.co-rec-add-item:hover { border-color:var(--inv-indigo); color:var(--inv-indigo); }

.co-rec-totals { background:var(--inv-s1); border-radius:8px; padding:14px 16px; font-size:13px; display:flex; flex-direction:column; gap:6px; align-items:flex-end; }
.co-rec-totals-row { display:flex; gap:32px; width:240px; justify-content:space-between; }
.co-rec-totals-label { color:var(--inv-s5); }
.co-rec-totals-value { font-weight:600; color:var(--inv-navy); }
.co-rec-totals-total .co-rec-totals-label { font-weight:700; color:var(--inv-navy); font-size:15px; }
...
.co-rec-client-search-row input:focus { outline:none; border-color:var(--inv-indigo); box-shadow:0 0 0 3px rgba(99,102,241,.1); }
.co-rec-client-results { position:absolute; top:100%; left:0; right:0; z-index:20; background:#fff; border:1.5px solid var(--inv-s2); border-top:none; border-radius:0 0 8px 8px; max-height:200px; overflow-y:auto; box-shadow:0 4px 16px rgba(15,23,42,.1); }
.co-rec-client-opt { padding:10px 14px; font-size:14px; cursor:pointer; border-bottom:1px solid var(--inv-s2); }
.co-rec-client-opt:last-child { border-bottom:none; }
.co-rec-client-opt:hover { background:var(--inv-s1); }
.co-rec-client-selected { display:flex; align-items:center; justify-content:space-between; padding:9px 12px; border:1.5px solid var(--inv-s2); border-radius:8px; background:#fff; }
.co-rec-client-clear { background:none; border:none; color:var(--inv-s3); cursor:pointer; font-size:16px; padding:0; }
.co-rec-new-client { border:1.5px solid var(--inv-indigo); border-radius:8px; padding:14px; margin-top:8px; background:#EEF2FF15; }
.co-rec-new-client-title { font-size:12px; font-weight:700; color:var(--inv-indigo); text-transform:uppercase; letter-spacing:.5px; margin:0 0 12px; }
...
.co-rec-end-toggle label { display:flex; align-items:center; gap:6px; font-size:13px; font-weight:500; color:var(--inv-s6); cursor:pointer; }
`;
			...
			{ pages > 1 && (
				<div className="co-rec-pager">
					...
					{ Array.from( { length: pages }, ( _, i ) => i + 1 )
						...
						.map( ( p, i ) =>
							p === '…' ? (
								<span key={ `ellipsis-${ i }` } style={ { padding: '0 4px', color: 'var(--inv-s4)' } }>…</span>
							) : (
								<button
									key={ p }
									type="button"
